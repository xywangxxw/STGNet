...
def trajectory_visualization(data: Batch, traj_output: torch.tensor, is_test: torch.bool = False) -> None:
    ...
    num_modes = traj_output.size(1)
    traj_output = traj_output.detach()
    traj_output = unbatch(traj_output, agent_batch)
    ...
    for i in range(batch_size):
        ...
        lane_ids = map_api.get_lane_ids_in_xy_bbox((x_min + x_max) / 2, (y_min + y_max) / 2, city_name[i],
                                                   max((x_max - x_min) / 2, (y_max - y_min) / 2) + 5)
        ...
        if is_test:
            ...
            plt.savefig(f'output/visualization/{data["scenario_id"][i]}.png')
        else:
