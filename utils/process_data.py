...
def generate_similarity_edge_index(similarity_matrix: torch.Tensor, k: int) -> torch.Tensor:
    N = similarity_matrix.size(0)
    k = min(k, N)

    topk_values, topk_indices = torch.topk(similarity_matrix, k, dim=1, largest=True, sorted=True)

    mask = torch.zeros_like(similarity_matrix, dtype=torch.bool)
    mask.scatter_(1, topk_indices, True)

    filtered_M = similarity_matrix * mask

    return filtered_M

def construct_hypergraph_edge_index_from_sparse_edge_index(sparse_edge_index: torch.Tensor) -> torch.Tensor:
    row, col = sparse_edge_index
    arange = torch.arange(row.size(0), device=sparse_edge_index.device)

    hyperedge_index = torch.stack([torch.cat([row, col]), torch.cat([arange, arange])], dim=0)
    return hyperedge_index


def expand_batch(batch: torch.Tensor, repeats: Tuple[int, ...]) -> torch.Tensor:
    batch_flat = batch.view(batch.size(0), *([1] * len(repeats)))
    batch_expanded = batch_flat.repeat(1, *repeats).reshape(-1)
