...
class MapEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.c_emb_layer = MLP(input_dim=1, hidden_dim=hidden_dim, output_dim=hidden_dim)
        self.l_emb_layer = MLP(input_dim=4, hidden_dim=hidden_dim, output_dim=hidden_dim)

        self.c2l_emb_layer = MLP(input_dim=3, hidden_dim=hidden_dim, output_dim=hidden_dim)
        self.l2l_emb_layer = MLP(input_dim=7, hidden_dim=hidden_dim, output_dim=hidden_dim)
    ...
    def forward(self, data: Batch) -> Dict:
        ...
        c_embs = self.c_emb_layer(input=c_length.unsqueeze(-1))
        ...
        l_input = torch.stack([l_length, l_is_intersection, l_turn_direction, l_traffic_control], dim=-1)
        l_embs = self.l_emb_layer(input=l_input)
        ...
        l_embs = self.c2l_attn_layer(x = [c_embs, l_embs], edge_index = c2l_edge_index, edge_attr = c2l_edge_attr_embs)

        l_embs = self.l2l_attn_layer(x = l_embs, edge_index = l2l_edge_index, edge_attr = l2l_edge_attr_embs)

        return {'l_embs': l_embs}
